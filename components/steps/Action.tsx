...
export default function Action({ selected, setSelected }: ActionProps) {
  ...
  return (
    <>
      <main id="action" className="modal__content content--action" role="main"
  aria-labelledby="action-title">
      ...
      >
        ...
        <section className="options options--action" role="group"
    aria-labelledby="action-title">
          {options.map((option, idx) => (
            ...
            >
              ...
              <span 
              id={`action-description-${idx}`}
              className="sr-only"
            >
              {option.description}. {selected[idx] ? 'Selected' : 'Not selected'}
            </span>
            </div>
          ))}
        </section>
